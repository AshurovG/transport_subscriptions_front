...
import {
  useCurrentApplicationDate,
  useSubscripitonsFromApplication,
  setCurrentApplicationDateAction,
  ...
  const deleteApplication = async () => {
    try {
      const response = await axios(
        `http://localhost:8000/applications/delete`,
        {
          method: "DELETE",
          withCredentials: true,
        }
        ...
        {flag ? (
          ...
          <>
            ...
            <h5 className={styles["application__page-subtitle"]}>
              У вас есть возможность удалять абонементы из заявки, удалить всю
              заявку или отправить заявку на проверку модераторам!
            </h5>
          </>
        )}
        <div>
          {!flag ? (
            <>
              <h3 className={styles["application__page-info-title"]}>
              ...
              <SubscriptionsTable
                flag={false}
                subscriptions={subscripitonsFromApplication}
                className={styles["application__page-table"]}
              />
              <h3
                className={styles["application__page-text"]}
                style={{ marginTop: 30 }}
              >
              ...
              <div className={styles["application__page-info-btns"]}>
                ...
                <Button
                  ...
                  className={styles["application__page-info-btn"]}
                >
                  Отправить
                </Button>
                <Button
            ...
            <SubscriptionsTable
              flag={true}
              subscriptions={currentSubsription}
              className={styles["application__page-table"]}
            />
          )}
        </div>
