...
const SubscriptionsPage: React.FC = () => {
  ...
  React.useEffect(() => {
    dispatch(
      setLinksMapDataAction(
        new Map<string, string>([["Абонементы", "/subscriptions"]])
      )
    );
  }, []);
  ...
  const getSubscriptions = async () => {
    ...
    try {
      ...
      if (categoryValue && categoryValue !== "Все категории") {
        const filteredArray = mockSubscriptions.filter(
          (mockSubscription) => mockSubscription.categoryTitle === categoryValue
        );
        ...
        const filteredArray = mockSubscriptions.filter(
          (mockSubscription) => mockSubscription.price <= priceValues[1]
        );
        dispatch(setSubscriptionsAction(filteredArray));
      } else {
  ...
  const postSubscriptionToApplication = async (id: number) => {
    try {
      const response = await axios(
        `http://localhost:8000/subscriptions/${id}/post`,
        {
          method: "POST",
          withCredentials: true,
        }
  ...
  const handleCategorySelect = (eventKey: string | null) => {
    if (eventKey !== null) {
      const selectedCategory = dropdownCategories.find(
        (category) => category.id === parseInt(eventKey, 10)
      );
      if (selectedCategory) {
    ...
    <div className={styles["main__page"]}>
      ...
      <div className={styles["main__page-wrapper"]}>
        ...
        <h5 className={styles["main__page-subtitle"]}>
          Также вы может найти абонемент по определенным фильтрам, которые
          представлены ниже!
        </h5>
        ...
        <Form className={styles["form"]} onSubmit={handleFormSubmit}>
          <div className={styles.form__wrapper}>
            ...
            <Button
              className={styles["form__mobile-button"]}
              onClick={() => handleSearchButtonClick()}
            >
              Найти
            </Button>
        ...
        <div className={styles["main__page-cards"]}>
          {subscriptions.map((subscription: Subscription) => (
            <OneCard
              ...
              onButtonClick={() =>
                postSubscriptionToApplication(subscription.id)
              }
              title={subscription.title}
              category={subscription.categoryTitle}
