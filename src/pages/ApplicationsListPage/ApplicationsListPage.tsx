  ...
  React.useEffect(() => {
    dispatch(
      setLinksMapDataAction(
        new Map<string, string>([["Заявки", "/applications"]])
      )
    );
    getAllApplications();
  }, []);
  ...
  return (
    <div className={styles.applications__page}>
      ...
      <div className={styles["applications__page-wrapper"]}>
        ...
        <h1 className={styles["applications__page-title"]}>
          История ваших заявок
        </h1>
        <h5 className={styles["applications__page-subtitle"]}>
          На этой странице расположена вся история ваших заявок. Вы можете
          посмотреть информацию о каждой заявке, а также добавленные в нее
          абонементы!
        </h5>
