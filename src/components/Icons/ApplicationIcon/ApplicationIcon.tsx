...
const ApplicationIcon: React.FC<ButtonProps> = ({onClick}) => {
  const subscriptionsFromApplications = useSubscripitonsFromApplication();
  return <svg onClick={() => {subscriptionsFromApplications.length !== 0 && onClick()}} style={{cursor: "pointer"}} fill={subscriptionsFromApplications.length !== 0 ? "#3D348B" : "#D9D9D9"} width="30px" height="30px" viewBox="0 0 16 16" id="register-16px" xmlns="http://www.w3.org/2000/svg">
